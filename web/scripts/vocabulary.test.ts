...
describe('vocabulary generator', () => {
  ...
  it('fails on a limits row the document lacks', () => {
    const rows = Object.fromEntries(
      Object.entries(ROWS).filter(([name]) => name !== 'RecordingRow'),
    )
  ...
  it('publishes an array of enum values as its item cap', () => {
    const rows = {
      ...
      TuneRow: {
        properties: {
          ...ROWS.TuneRow.properties,
          modes: {
            anyOf: [
              { type: 'array', maxItems: 4, items: { $ref: '#/components/schemas/Mode' } },
              { type: 'null' },
            ],
          },
        },
