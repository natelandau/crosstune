...
import { createSong, deleteSong, updateSong } from './songs'
...
describe('writes to one row in one millisecond', () => {
  ...
  it('stamps each upsert and delete later than the last', async () => {
    ...
    const updated = await db.songs.get(songId)
    expect(updated?.updated_at).toBe('2026-09-11T10:00:00.001Z')
    expect((await pendingFor(db, 'songs', songId))?.updated_at).toBe(updated?.updated_at)

    await deleteSong(db, songId)
    const deleted = await db.songs.get(songId)
    expect(deleted?.updated_at).toBe('2026-09-11T10:00:00.002Z')
    expect(deleted?.deleted_at).toBe(deleted?.updated_at)
    expect((await pendingFor(db, 'songs', songId))?.updated_at).toBe(deleted?.updated_at)
  })
})
