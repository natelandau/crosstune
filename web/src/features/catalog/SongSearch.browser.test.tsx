...
describe('SongSearch', () => {
  ...
  it("says an archived song is archived after the caller's words", async () => {
    await setArchived(db, hen.userSongId, true)
    renderIonic(<Host />, { db })
    await search().fill('cluck')
    await expect
  ...
  it("marks a taken song with the caller's note instead of offering it", async () => {
    renderIonic(<Host taken={new Set([joy.userSongId])} takenLabel="Already filed" />, { db })
  ...
  it('reports a taken song neither from a tap nor from Enter', async () => {
    const onPick = vi.fn()
    renderIonic(
      <Host taken={new Set([joy.userSongId])} takenLabel="Already filed" onPick={onPick} />,
      { db },
    )
    await search().fill("Soldier's Joy")
    await expect.element(page.getByText('Already filed')).toBeVisible()
    await page.getByRole('heading', { name: "Soldier's Joy" }).click({ force: true })
    // Enter with a lone match resolves to that song, so a taken one has to be refused here too.
    await search().fill("Soldier's Joy")
    await expect.element(page.getByText('Already filed')).toBeVisible()
  ...
  it('leaves the field alone on a pick, so only the caller empties it', async () => {
    ...
    expect(onPick.mock.calls[0]?.[0].song.title).toBe('Cluck Old Hen')
    await expect.poll(held).toBe('cluck')
  ...
  it('offers to create a song by the typed title', async () => {
    const onCreate = vi.fn()
    renderIonic(<Host onCreate={onCreate} />, { db })
    await search().fill('Sally Goodin')
    await expect.element(page.getByText('No song called "Sally Goodin"')).toBeVisible()
