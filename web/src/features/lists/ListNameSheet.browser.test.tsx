...
describe('ListNameSheet', () => {
  ...
  it('requires a name and says so under the field', async () => {
    ...
    await page.getByLabelText('List name').fill('   ')
    await page.getByRole('button', { name: 'Create', exact: true }).click()
    await expect.element(page.getByRole('alert')).toHaveTextContent('A list needs a name')
  ...
  it('renames with the current name filled in, and saves on Enter', async () => {
    ...
    await expect.element(page.getByText('Rename list')).toBeVisible()
  ...
  it('discards the name on Cancel and reports the close once', async () => {
    ...
    await page.getByLabelText('List name').fill('Draft')
  ...
  it('clears a stale save failure as the name is retyped', async () => {
    ...
    await expect.element(page.getByRole('alert')).toHaveTextContent('List not found')
    await page.getByLabelText('List name').fill('Ghost jam')
