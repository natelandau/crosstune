...
describe('SongPickerSheet', () => {
  ...
  it('finds archived songs and says so in the row name', async () => {
  ...
  it('offers to create a song by the typed title and hands it to the caller', async () => {
  ...
  it('offers nothing while the catalog is still being read', async () => {
    ...
    // must not be answered as though the catalog held no such song.
    vi.spyOn(db.songs, 'toArray').mockReturnValue(new Promise(() => {}) as never)
    renderIonic(<Host onCreate={onCreate} />, { db })
    await search().fill("Soldier's Joy")
    await new Promise((resolve) => setTimeout(resolve, 100))
    expect(page.getByText('No song called "Soldier\'s Joy"').elements()).toHaveLength(0)
    expect(page.getByRole('button', { name: 'Add "Soldier\'s Joy"' }).elements()).toHaveLength(0)
    await userEvent.keyboard('{Enter}')
    await new Promise((resolve) => setTimeout(resolve, 100))
  ...
  it('adds a song once from two taps in the same tick', async () => {
    renderIonic(<Host />, { db })
    await search().fill('cluck')
