...
export function SongPickerSheet({
  ...
  open: boolean
  listId: string
  /** The user song ids already in the list, from the screen's own query rather than a second one. */
  taken: ReadonlySet<string>
  onClose: () => void
  /** The musician chose to create a song by this title; the picker has already closed. */
  ...
  // Songs whose add is in flight. A ref, because two taps in one tick both read the same state.
  ...
  const pick = (entry: CatalogEntry) => {
    ...
    run(async () => {
      try {
        ...
        // Adding several songs means the sheet must close on Done whatever is in flight, so a
      ...
      {open ? (
        <>
          <SongSearch
            ...
            keepFocus
          />
          {error ? <InlineError className="px-8 pt-2">{error}</InlineError> : null}
          {!query.trim() ? (
            <p className="type-footnote px-8 pt-4">Search the catalog to add songs.</p>
          ) : null}
        </>
      ) : null}
    </Sheet>
