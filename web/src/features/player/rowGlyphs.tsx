...
/**
 * The slot before a row's title, sized to the row's touch target whatever it holds. Carries
 * Ionic's own `start` slot name, since it is always a row's leading control.
 */
export function Slot({ children }: { children?: ReactNode }) {
  return (
    <span slot="start" className="flex size-11 shrink-0 items-center justify-center">
      {children}
    </span>
  )
}
