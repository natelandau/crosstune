...
/**
 * A tune's status as a rail of capsules, the same control the key rail uses directly below it,
 * so the two read as two fields rather than as a control and a toolbar.
 *
 * A tune always has a status, so this one never clears: pressing the chosen capsule leaves it
...
type ChooserProps =
  ...
  | {
       ...
       * setting a tune's own status. */
      includeAll: true
    }

export function StatusChooser({ value, onChange, includeAll = false }: ChooserProps) {
  // A status a tune holds but this client cannot read still shows as want to learn; a filter
  // set to all matches none of the three and leaves every capsule unpressed but All.
  const current = isTuneStatus(value) ? value : includeAll ? null : 'want_to_learn'
  return (
    <Rail label="Status">
      {includeAll ? (
        <Capsule
