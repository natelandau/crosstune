...
export const DELETE_TUNE_TITLE = 'Delete tune?'
...
export function deleteTuneMessage(title: string, recordings: readonly Recorded[]): string {
  return message(`"${title}"`, 'its', recordings)
}

/** The bulk confirmation, whose subject counts the selection: "12 tunes". */
export function deleteTunesMessage(subject: string, recordings: readonly Recorded[]): string {
