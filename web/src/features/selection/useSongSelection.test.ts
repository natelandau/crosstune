...
function setup(initial: { ids?: readonly string[]; active?: boolean } = {}) {
  return renderHook(({ ids, active }) => useSongSelection(ids, active), {
...
describe('useSongSelection', () => {
  ...
  it('selects all visible songs and deselects them', () => {
    const { result } = setup()
  ...
  it('selects a range downward from the last toggled song', () => {
    const { result } = setup()
    act(() => result.current.toggle('b'))
  ...
  it('selects a range upward from the last toggled song', () => {
    const { result } = setup()
  ...
  it('drops songs that are no longer visible', () => {
  ...
  it('keeps a song toggled just before selection mode starts', () => {
  ...
  it('clears when selection mode ends while the visible songs change', () => {
    const { result, rerender } = setup()
    act(() => result.current.toggleAll())
  ...
  it('selectAll selects every visible song', () => {
    const { result } = setup()
