...
export function createApiClient(options: ApiClientOptions): SyncApi {
  ...
  const client = createClient<paths>({
    // The dev and preview servers proxy /v1 on the same origin, so the baseUrl is
    // empty there; production sets VITE_API_URL. A browser resolves an empty baseUrl
    // against the page location on its own, but the fetch client here needs an
    // absolute URL, so mirror that resolution.
