...
Crosstune is a song catalog for folk musicians. A musician keeps the songs
they know and the songs they want to learn, and reaches a recording of any
song in two taps, with or without a network connection.
...
- Jam recordings land in a general voice memo app, mixed with everything else
  and hard to find.
- Song lists live on paper, sorted by key, with a separate list of songs to
  learn.
- A musician's own recording of a song and the streaming versions live in
  different apps.
- At a jam, a musician needs the list of songs they know and a fast reminder
...
1. Add a song with its key, tuning, and other attributes.
2. Mark it known, learning, or want to learn.
3. Paste links to recordings of it, or record it.
4. At the jam, filter the catalog by key, tap the song, hear it.
...
- Catalog. Songs with musical attributes (key, mode, tunings, genre, feel,
  time signature, part structure), lyrics, where and when they were learned,
  notes, and a status. Songs can be archived.
- Lists. Ordered, named lists such as a setlist. A song can be in many.
...
- Recordings. Record with the phone's microphone or upload audio. A
  recording is filed under a song or waits unfiled. It plays at once, uploads
...
- Private now, shared later. The schema separates a song from a user's
  relationship to it, so a shared catalog is a merge step, not a rewrite.
- Musical facets live on the song (key, mode, tuning, part structure). The
...
Two naming rules hold in the schema, the API, and every label: song, never
tune, and violin, never fiddle. Tuning values keep their traditional names.
